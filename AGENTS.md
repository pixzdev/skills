...
- **First activation (post-install):** `DETECT → INVENTORY → BASELINE → ADAPT → VERIFY → PERSIST → READY`. Probe: `python3 scripts/activation.py status` (exit 0 = ready, 10 = new/stale). No state → `init`, adapt behavior, verify at RUNTIME-ACTIVE depth, `mark-adapted --evidence …` (refused without evidence — "I have learned PixzFlow" is not evidence). State: `.pixz/adaptation-state.json` (`schemas/adaptation-state.schema.json`). Baseline records only observed facts — `scripts/doctor.py --write` **measures** what is locally measurable (state persistence, command execution, tools, contract reachability); what it cannot measure stays `unknown`, never invented. No hidden chain-of-thought in state.
- **Adoption assessment (auto):** `mark-adapted` automatically runs `scripts/assess.py` — adoption score 0–100, a sum of **named evidence-backed checks** (each printed PASS/FAIL; UNVERIFIED items score 0; gaps listed). It is NOT a model-quality or workflow-effectiveness metric — that is successor-benchmark territory. Reproduce anytime: `scripts/assess.py [--full]`. Situation report for ORIENT/compaction/handoff: `scripts/sitrep.py`.
- **Integrity & wiring:** installed copies are checkable against source digests (`activation.py verify-installed --runtime <rt>` — tamper/corruption = inspect before trusting); runtime contract wiring is mechanical (`scripts/hooks.py check|install --runtime <rt>`).
