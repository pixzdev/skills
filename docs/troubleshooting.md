...
- Check `registry.json#limits` (`max_skill_chain_depth=20` since 2.4.0). The orchestrator resolves to 4 mandatory nodes (17 with `--with-optional`), well under the cap; the cap exists to bound runaway chains, not normal resolution.
